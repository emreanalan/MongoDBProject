...
def select_zam_days():
    zam_days = []
    possible_days = list(range(7, len(dates) - 7))
    while True:
        if len(zam_days) >= 6 or not possible_days:
            break
        day = random.choice(possible_days)
...
print("🚀 ShopGroup'lar oluşturuluyor...\n")

for shopgroup_idx, group_shops in enumerate(collusion_groups, start=1):
    group_name = f"ShopGroup_{shopgroup_idx}"
    group_collection = db[group_name]
    group_collection.delete_many({})

    print(f"✨ {group_name} başlıyor...")

    bulk_ops = []  # Burada her ShopGroup için sıfırla
    ...
    print(f"- Lider: {leader_shop}")
    print(f"- Followerlar: {follower_shops}")
    print(f"- Manufacturerlar: {selected_manufacturers}")
    print(f"- Zam Günleri: {[d.strftime('%Y-%m-%d') for d in zam_real_dates]}")
    print(f"- Gecikmeler: {follower_delays}\n")

    product_profits = {}
    leader_previous_prices = {}

    for day_idx, current_date in enumerate(dates):
        doc = {
            "Date": current_date,
            "Shop Name": leader_shop,
            "Store Type": leader_shop,
            "Description": f"Product prices for {leader_shop} on {current_date.strftime('%Y-%m-%d')}"
        }
        ...
        for manu in selected_manufacturers:
            ...
            for i, prod_key in enumerate(product_keys):
                ...
                if day_idx == 0 or day_idx in zam_days:
                    price = manufacturer_price * (1 + profit_pct)
                    leader_previous_prices[pname] = price
                else:
                    price = leader_previous_prices[pname]
                manu_doc[f"Product {i+1}"] = pname
                manu_doc[f"Product {i+1} Price"] = f"{price:,.2f} TL"
        ...
        bulk_ops.append(pymongo.UpdateOne({"Date": current_date, "Shop Name": leader_shop}, {"$set": doc}, upsert=True))

    for follower_shop in follower_shops:
        delay = follower_delays[follower_shop]
        follower_previous_prices = {}
        ...
        for day_idx, current_date in enumerate(dates):
            ...
            for manu in selected_manufacturers:
                ...
                for i, prod_key in enumerate(product_keys):
                    ...
                    manufacturer_price = float(cost_str.replace(",", "").replace(" TL", ""))
                    profit_pct = product_profits[pname] / 100
                    if delayed_idx == 0 or delayed_idx in zam_days:
                        price = manufacturer_price * (1 + profit_pct)
                        follower_previous_prices[pname] = price
                    else:
                        price = follower_previous_prices.get(pname, manufacturer_price * (1 + profit_pct))
                    manu_doc[f"Product {i+1}"] = pname
                    manu_doc[f"Product {i+1} Price"] = f"{price:,.2f} TL"
            ...
            bulk_ops.append(pymongo.UpdateOne({"Date": current_date, "Shop Name": follower_shop}, {"$set": doc}, upsert=True))

    if bulk_ops:
        batch_size = 30
        for i in range(0, len(bulk_ops), batch_size):
            batch = bulk_ops[i:i+batch_size]
            group_collection.bulk_write(batch)

print("\n✅ Tüm ShopGroup'lar başarıyla oluşturuldu!")
